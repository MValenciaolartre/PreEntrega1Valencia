...
const ItemListContainer = ({ greeting }) => {
  ...
  return (
    ...
    {productos.map((producto) => (
      <Link key={producto.id} to={`/item/${producto.id}`} className="block">
        <div className="p-2 transition rounded-md user-card hover:scale-100 group bg-slate-300 max-w-xs">
          <div className="overflow-hidden aspect-w-16 aspect-h-9" style={{ height: "300px", width: "300px" }}>
            <img
              className="w-full h-full object-cover transition-all duration-500 rounded-md group-hover:grayscale-0 group-hover:scale-150"
              src={producto.imagen}
              alt="card image"
            />
          </div>
